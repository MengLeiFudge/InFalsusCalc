...
internal sealed partial class KeyRecipeSolver
{
    ...
    public KeyRecipeSolver(Catalog catalog, Recipe recipe, Func<int> threadBudget, double slice, CancellationToken cancellation, bool proveConfidence = false)
    {
        ...
        Result = Load(catalog, recipe) ?? new RecipeResult
        {
            Recipe = recipe.Id,
        ...
        foreach (uint mask in Result.InfeasibleRegions)
            solved[(mask, 0)] = null;
        foreach (var layer in Result.InfeasibleRegionLayers)
            foreach (uint mask in layer.Value)
                solved[(mask, layer.Key)] = null;
        foreach (CardTemplate stored in Result.Cards.Values)
        {
            ...
            if (solved.TryGetValue(solvedKey, out CardTemplate? existing) && existing is null)
                throw new InvalidDataException($"配方{recipe.Id}的布局与不可行区域证明冲突：{mask}/p{card.Strikes}。");
        ...
        {
            foreach (BoardCell c in area.Cells)
                target[(c.Position, c.Color)] = bit++;
            foreach (RegionEffect e in area.Effects)
            {
                if (e.Kind == 8)
                {
                    effects[index, 16] += e.Arguments[0].Value;
                    effects[index, 17] += e.Arguments[1].Value;
                }
                else if (e.Kind != 1)
                    effects[index, e.Kind] += e.Arguments[0].Value;
            }
        }
        ...
        foreach (Shape shape in catalog.Data.Shapes)
        {
            ...
            foreach (Hex anchor in cells)
            {
                ...
                if (occupied.Any(c => !cellIndex.ContainsKey(c)))
                    continue;
                ...
                {
                    regions |= 1u << group.Key;
                    regionMaxCover[group.Key] = Math.Max(regionMaxCover[group.Key], group.Count());
                }
        ...
        {
            regionMinimum[i] = CoverMinimum(i, offset, regionMinimum[i]);
            offset += recipe.Areas[i].Cells.Length;
        }
        ...
        for (int region = 0; region < recipe.Areas.Length; region++)
            if (recipe.Areas[region].Cells.Length > 1)
                regionMinimum[region] = Math.Max(regionMinimum[region], GeometryMinimum(region));
        ...
        if (!independent)
        {
            ...
            while (pending != 0)
            {
                ...
                if (areas.Sum(i => recipe.Areas[i].Cells.Length) > 63 || areas.Length > 10)
                    continue;
                Dictionary<uint, int[]> costs = new()
                {
                    [0] = new int[outsideBudget + 1]
                };
        ...
        if (safeParts.Count > 64)
            throw new InvalidDataException("安全区连通分量超过当前位集容量。");
        safeContacts = new ulong[cells.Length];
        regionSafeParts = new ulong[recipe.Areas.Length];
        for (int part = 0; part < safeParts.Count; part++)
        {
            foreach (Hex c in safeParts[part].SelectMany(c => Hex.Directions.Select(c.Add).Append(c)))
                if (cellIndex.TryGetValue(c, out int index))
                    safeContacts[index] |= 1UL << part;
            for (int i = 0; i < recipe.Areas.Length; i++)
                if (recipe.Areas[i].Cells.Any(c => safeParts[part].Contains(c.Position)))
                    regionSafeParts[i] |= 1UL << part;
        }
    ...
    public void Run(int[]? requested, string? goal, bool retry)
    {
            ...
            .ThenBy(g => g.Key))
        {
            ...
            if (!Result.Groups.TryGetValue(groupId, out GroupState? group))
                Result.Groups[groupId] = group = new GroupState { Key = tuple, Strikes = currentStrikes };
            else if (group.Strikes != currentStrikes)
                throw new InvalidDataException($"配方{recipe.Id}/{groupId}惩罚层不一致。");
            foreach (string name in requestedGoals.OrderBy(n => Attempted(Result, groupId, n) ? 1 : 0).ToArray())
            {
                if (Done(Result, groupId, name))
                    continue;
                if (retry != Attempted(Result, groupId, name))
                    continue;
                ...
                long before = checks;
                goalClock = watch;
                target = $"key={key} p{currentStrikes} {name}";
                progress = target + " 区域收益排序";
                ...
                RegionSet[] ordered = sets.Where(s =>
                    {
                        if (savedUpper is null)
                            return true;
                        var value = Objective(s, g, currentStrikes);
                        if (legacyPrimaryOptimal)
                            return value.Primary == savedUpper.Value;
                ...
                (int Primary, int Secondary) upper = default;
                var initialUpper = ordered.Length == 0 ? default : Objective(ordered[0], g, currentStrikes);
                runningGoal = new KeyGoalState
                {
                    Status = "UNKNOWN",
                    SearchPolicy = Result.Policy,
                    UpperBound = initialUpper.Primary,
                    SecondaryUpperBound = initialUpper.Secondary
                };
                ...
                if (group.Best.TryGetValue(name, out string? incumbentId) && Result.Cards.TryGetValue(incumbentId, out CardTemplate? incumbent))
                {
                    ...
                    while (true)
                    {
                        ...
                        {
                            foreach (RegionSet impossible in pending)
                                solved[(impossible.Mask, currentStrikes)] = null;
                            continue;
                        }
                ...
                if (!unresolved && (!thresholdClosed || g == 2))
                    ...
                    {
                        if (foundTotal.HasValue && batch.Key.Primary < foundTotal.Value)
                            break;
                        if (SearchBudget <= 0)
                        {
                            unresolved = true;
                        ...
                        if (card is null)
                        {
                            ...
                            if (card is null)
                                foreach (RegionSet impossible in alternatives)
                                    solved[(impossible.Mask, currentStrikes)] = null;
                        }
                        if (card is null)
                            continue;
    ...
    private int GeometryMinimum(int region, int? budget = null)
    {
        ...
        CpModel model = new();
        BoolVar[] take = options.Select((_, i) => model.NewBoolVar($"local{i}")).ToArray();
        ...
        foreach (var group in options.SelectMany((p, i) => p.Cells.Select(c => (c, i))).GroupBy(p => p.c))
        {
            ...
            model.Add(LinearExpr.Sum(group.Select(p => take[p.i])) <= 1).OnlyEnforceIf(over.Not());
            repeated.Add(over);
        }
        ...
        CpSolver solver = new()
        {
            StringParameters = "max_time_in_seconds:0.3 num_search_workers:1 cp_model_probing_level:0"
        };
    ...
    private int MinimumCover(ulong[] covers, int count, int lower)
    {
        ...
        bool Search(ulong remaining, int depth, Dictionary<ulong, int> failed)
        {
            ...
            if (depth == 0 || System.Numerics.BitOperations.PopCount(remaining) > depth * maxCover)
                return false;
            if (failed.TryGetValue(remaining, out int old) && old >= depth)
                return false;
            ulong[]? next = null;
    ...
    private int MinimumCover((ulong Mask, bool Outside)[] patterns, int count, int budget)
    {
        ...
        bool Search(ulong remaining, int depth, int outside, Dictionary<(ulong, int), int> failed)
        {
            ...
            if (depth == 0 || System.Numerics.BitOperations.PopCount(remaining) > depth * maximum)
                return false;
            if (failed.TryGetValue((remaining, outside), out int old) && old >= depth)
                return false;
    ...
    private static void NormalizeBest(RecipeResult result, GroupState group)
    {
        ...
        if (cards.Length == 0)
            return;
        foreach (string goal in group.Best.Keys.Where(name => name != "total").ToArray())
            group.Best[goal] = Craft.BestForGoal(cards, goal).Id;
    ...
    private static void RecordCandidate(RecipeResult result, GroupState group, string goal, CardTemplate card)
    {
        ...
        if (goal == "total")
        {
            if (group.Best.TryGetValue(goal, out string? oldTotal))
                group.TotalBest = group.TotalBest.Append(oldTotal).ToArray();
            group.TotalBest = group.TotalBest.Append(card.Id).Distinct().ToArray();
            group.Best[goal] = card.Id;
        }
        else if (!group.Best.TryGetValue(goal, out string? oldId) || !result.Cards.TryGetValue(oldId, out CardTemplate? old))
            group.Best[goal] = card.Id;
        else
            group.Best[goal] = Craft.BestForGoal([old, card], goal).Id;
        NormalizeBest(result, group);
    }
    ...
    public void Save()
    {
        ...
        foreach (var pair in Result.Groups)
        {
            if (!previous.Groups.TryGetValue(pair.Key, out GroupState? group))
                previous.Groups[pair.Key] = group = new GroupState { Key = pair.Value.Key, Strikes = pair.Value.Strikes };
            ...
            foreach (var goal in pair.Value.Goals)
            {
                ...
                if (oldGoal?.Status == "OPTIMAL" && oldGoal.ObjectiveComplete && goal.Value.Status != "OPTIMAL"
                    || oldGoal?.Status == "CONFIDENCE" && goal.Value.Status == "UNKNOWN")
                    continue;
                ...
                else if (goal.Value.Status == "INFEASIBLE")
                    group.Best.Remove(goal.Key);
            }
        }
        ...
        foreach (var cards in previous.Cards.Values.GroupBy(c => $"{c.Strikes}:{c.Power}:{c.Fortitude}:" + string.Join(',', c.Active.Order())))
        {
            ...
            foreach (CardTemplate card in cards)
                representatives[card.Id] = id;
        }
        foreach (GroupState group in previous.Groups.Values)
        {
            foreach (string name in group.Best.Keys.ToArray())
                group.Best[name] = representatives[group.Best[name]];
    ...
    private (CardTemplate? Card, CpSolverStatus Status) Feasible(RegionSet[] sets, int[] key)
    {
        ...
        checks++;
        if (proveConfidence && sets.Length > 24)
        ...
        if (seed is not null)
            return (seed, CpSolverStatus.Feasible);
        var cover = Geometry(sets, key, GeometryMode.Cover);
        if (cover.Card is not null || cover.Status == CpSolverStatus.Infeasible)
            return cover;
        if (recipe.Areas.Any(a => a.Cells.Length > 1) && sets.Length > 1)
            foreach (RegionSet candidate in sets.OrderBy(s => s.Cost).Take(8))
            {
                ...
                if (direct.Card is not null)
                    return direct;
            }
        var small = Geometry(sets, key, GeometryMode.Compact);
        if (small.Card is not null)
            return small;
        return Geometry(sets, key, GeometryMode.Full);
    }
    ...
    {
        if (localPatterns.TryGetValue(region, out var cached))
            return cached;
        ...
        List<int[]> result = [];
        HashSet<string> seen = [];
        int[] occupied = new int[cells.Length];
        List<int> selected = [];
        int outside = 0, overlaps = 0;
        Stopwatch clock = Stopwatch.StartNew();
        int outsideTarget = 0, quota = 0;
        double deadline = 0;
        ...
        {
            if (clock.Elapsed.TotalSeconds > deadline || result.Count >= quota || SearchBudget <= 0 || cancellation.IsCancellationRequested)
                return;
            if (covered == full)
            {
                if (outside != outsideTarget)
                    return;
                int[] ids = selected.Order().ToArray();
                if (seen.Add(string.Join(',', ids)))
                    result.Add(ids);
                return;
            }
            if (depth == 0)
            ...
            foreach (int i in next!)
            {
                ...
                if (clock.Elapsed.TotalSeconds > deadline || result.Count >= quota)
                    break;
            }
        }
        for (outsideTarget = 0; outsideTarget <= outsideBudget; outsideTarget++)
        {
            quota = result.Count + 64;
            deadline = clock.Elapsed.TotalSeconds + .15;
            for (int count = regionMinimum[region]; count <= Math.Min(limit, regionMinimum[region] + 2) && result.Count < quota; count++)
                Search(0, count);
        }
        ...
        {
            CpModel model = new();
            BoolVar[] take = indices.Select(i => model.NewBoolVar($"local{i}")).ToArray();
            for (int b = 0; b < size; b++)
                model.Add(LinearExpr.Sum(take.Where((_, j) => (masks[indices[j]] & (1u << b)) != 0)) >= 1);
            model.Add(LinearExpr.Sum(take) <= limit);
            ...
            for (int attempt = 0; attempt < 2; attempt++)
            {
                ...
                CpSolver solver = new()
                {
                    StringParameters = FormattableString.Invariant($"max_time_in_seconds:{Math.Min(1, SearchBudget)} num_search_workers:{threads} cp_model_probing_level:0 linearization_level:0")
                };
                using CancellationTokenRegistration registration = cancellation.Register(solver.StopSearch);
                CpSolverStatus status = solver.Solve(model);
                cancellation.ThrowIfCancellationRequested();
    ...
    private CardTemplate? FindPatterns(RegionSet[] sets, double seconds)
    {
        ...
        {
            if (clock.Elapsed.TotalSeconds >= seconds || SearchBudget <= 0)
                break;
            ...
            if (regions.Any(i => Patterns(i).Count == 0))
                continue;
            ...
            foreach (int region in regions)
                foreach (Hex c in recipe.Areas[region].Cells.SelectMany(c => Hex.Directions.Select(c.Position.Add).Append(c.Position)))
                    if (cellIndex.TryGetValue(c, out int index))
                        near[index] = near.GetValueOrDefault(index) | (1u << region);
            ...
            bool Search(int at)
            {
                if (clock.Elapsed.TotalSeconds >= until || SearchBudget <= 0 || cancellation.IsCancellationRequested)
                    return false;
                ...
                if (Enumerable.Range(offsets[region], recipe.Areas[region].Cells.Length).All(b => covered[b] > 0))
                    return Search(at + 1);
                ...
                foreach (int[] pattern in orderedPatterns[region])
                {
                    ...
                    if (!stackOk || selected.Count + added.Length + after > countLimit || outside + added.Count(i => pieces[i].Outside) > maxOutside)
                        continue;
                    foreach (int i in added)
                    {
                        if (pieces[i].Outside)
                            outside++;
                        foreach (int c in pieces[i].Cells)
                        if (occupied[c]++ == 1)
                            overlaps++;
                        foreach (int b in pieces[i].Matches)
                            covered[b]++;
                        selected.Add(i);
                        selectedSet.Add(i);
                    }
                    if (overlaps <= maxOverlap && Search(at + 1))
                        return true;
                    foreach (int i in added)
                    {
                        if (pieces[i].Outside)
                    ...
                    if (clock.Elapsed.TotalSeconds >= until)
                        break;
                }
                return false;
            }
        ...
        foreach (RegionSet set in sets.OrderBy(s => s.Cost).ThenByDescending(s => s.Power + s.Fortitude))
        {
            ...
            for (int i = 0; i < components.Count; i++)
                foreach (Hex c in components[i].SelectMany(c => Hex.Directions.Select(c.Add).Append(c)))
                    if (cellIndex.TryGetValue(c, out int index))
                        near[index] = near.GetValueOrDefault(index) | (1UL << i);
            ...
            List<int> chosen = [];
            int outside = 0, overlaps = 0, visited = 0;
            CardTemplate? answer = null;
            bool Fits(int index)
            {
                Piece p = pieces[index];
                if (p.Outside && outside >= maxOutside || chosen.Contains(index))
                    return false;
                if (p.Cells.Any(c => occupied[c] >= Craft.MaxStack))
                    return false;
                if (overlaps + p.Cells.Count(c => occupied[c] == 1) > maxOverlap)
                    return false;
                foreach (var g in p.Matches.Where(b => covered[b] == 0).GroupBy(b => bitArea[b]))
                    if ((set.Mask & (1u << g.Key)) == 0 && areaCovered[g.Key] + g.Count() == recipe.Areas[g.Key].Cells.Length)
                        return false;
                return true;
            }
            ...
            bool Search()
            {
                if (++visited % 128 == 0 && (clock.Elapsed.TotalSeconds >= until || cancellation.IsCancellationRequested))
                    return false;
                int[]? next = null;
                for (int b = 0; b < required.Length; b++)
                {
                    if (!required[b] || covered[b] != 0)
                        continue;
                    int[] possible = byBit[b].Where(Fits).ToArray();
                    if (possible.Length == 0)
                        return false;
                    if (next is null || possible.Length < next.Length)
                        next = possible;
                }
                if (next is null)
                {
                    ...
                    for (int i = 0; i < groups.Count; i++)
                        foreach (Hex c in groups[i].SelectMany(c => Hex.Directions.Select(c.Add).Append(c)))
                            if (cellIndex.TryGetValue(c, out int index))
                                adjacent[index] = adjacent.GetValueOrDefault(index) | (1UL << i);
                    ...
                    .Where(i => (set.Mask & (1u << i)) != 0).Sum(i => (recipe.Areas[i].Cells.Length - areaCovered[i] + regionMaxCover[i] - 1) / regionMaxCover[i]) > maxCount)
                    return false;
            ...
            {
                if (clock.Elapsed.TotalSeconds >= seconds || clock.Elapsed.TotalSeconds - setStart > .8)
                    break;
                if (!Fits(bridge))
                    continue;
    ...
    private (CardTemplate? Card, CpSolverStatus Status) Geometry(RegionSet[] sets, int[] key, GeometryMode mode, double? seconds = null)
    {
        ...
        if (mode == GeometryMode.Full && proveConfidence && sets.Length > 8)
        {
            ...
            foreach (RegionSet[] chunk in chunks)
            {
                ...
                if (answer.Card is not null)
                    return answer;
                if (answer.Status == CpSolverStatus.Infeasible)
                {
                    foreach (RegionSet impossible in chunk)
                        solved[(impossible.Mask, currentStrikes)] = null;
                }
                else
                    unknown = true;
        ...
        if (mode == GeometryMode.Full && proveConfidence && sets.Length > 1)
        {
            ...
            if ((long)sets.Length * choiceCount > 200000)
            {
                ...
                foreach (RegionSet[] half in halves)
                {
                    ...
                    if (answer.Status == CpSolverStatus.Infeasible)
                    {
                        foreach (RegionSet impossible in half)
                            solved[(impossible.Mask, currentStrikes)] = null;
                    }
                    else
                        unknown = true;
        ...
        if (mode == GeometryMode.Full && sets.Length > 1 && Environment.GetEnvironmentVariable("INFALSUS_SPLIT") == "1")
        {
            ...
            foreach (var item in ordered)
            {
                ...
                if (answer.Status == CpSolverStatus.Infeasible)
                {
                    solved[(item.Set.Mask, currentStrikes)] = null;
                    Save();
                }
                else
                    unknown = true;
        ...
        for (int i = 0; i < forcedComponents.Count; i++)
            foreach (Hex cell in forcedComponents[i].SelectMany(c => Hex.Directions.Select(c.Add).Append(c)))
                if (cellIndex.TryGetValue(cell, out int index))
                    near[index] = near.GetValueOrDefault(index) | (1UL << i);
        ...
        for (int i = 0; i < choices.Length; i++)
        {
            ...
            foreach (int b in choices[i].Matches)
                matches[b].Add(selected[i]);
        }
        ...
        if (sets.Length is > 1 and <= 32)
        {
            ...
            for (int s = 0; s < sets.Length; s++)
            {
                ...
                if (connect && currentStrikes == 0)
                    AddSafeConnectionBound(model, selected, choices, set, cases[s]);
                HashSet<int> domain = prunedDomains[set.Mask].ToHashSet();
                for (int i = 0; i < selected.Length; i++)
                    if (!domain.Contains(indices[i]))
                    {
                        model.AddImplication(cases[s], selected[i].Not());
                        disabled++;
                    }
        ...
        else
        {
            ...
            for (int s = 0; s < sets.Length; s++)
                for (int i = 0; i < active.Length; i++)
                    activation[s, i] = (sets[s].Mask & (1u << i)) != 0 ? 1 : 0;
            model.AddAllowedAssignments(active).AddTuples(activation);
        }
        if (connect && sets.Length == 1 && currentStrikes == 0)
            AddSafeConnectionBound(model, selected, choices, sets[0]);
        ...
        {
            if (key[0] == 0 && kind is 16 or 17)
                continue;
            if (value < cap)
                model.Add(Effect(kind) == value);
            else
                model.Add(Effect(kind) >= value);
        }
        IntVar power = model.NewIntVar(1, 200000, "power");
        IntVar fortitude = model.NewIntVar(1, 200000, "fortitude");
        model.Add(power == Effect(3));
        model.Add(fortitude == Effect(4));
        var panels = sets.Select(s => (s.Power, s.Fortitude)).Distinct().ToArray();
        long[,] tuples = new long[panels.Length, 2];
        for (int i = 0; i < panels.Length; i++)
        {
            tuples[i, 0] = panels[i].Power;
            tuples[i, 1] = panels[i].Fortitude;
        }
        ...
        for (int i = 0; i < required.Length; i++)
        {
            ...
            foreach (BoardCell cell in recipe.Areas[i].Cells)
            {
                ...
                if (m.Count == 0)
                    ...
                    model.AddMaxEquality(filled, m);
                model.AddImplication(active[i], filled);
                missing.Add(filled.Not());
            }
        ...
        for (int c = 0; c < cells.Length; c++)
        {
            ...
            foreach (BoolVar particle in forcedOverlap[c])
                model.AddImplication(particle, over);
            overlap.Add(over);
        }
        ...
        if (connect)
            AddConnection(model, active, cover, forced, sets, splitPenalty);
        ...
        if (retry)
            model.Minimize(LinearExpr.Sum(selected));
        ...
        CardTemplate? Validate(int[] chosen)
        {
            ...
            if (!sets.Any(s => s.Mask == actual))
                throw new InvalidDataException($"区域集合约束不一致：{recipe.Id}/{actual}");
            ...
            if (connect && currentStrikes == 0)
                throw new InvalidDataException($"完整连接模型复核不一致：{recipe.Id}，惩罚{card.Strikes}。");
            return null;
        }
                ...
                while (watch.Elapsed.TotalSeconds < budget)
                {
                    ...
                    if (card is not null)
                    {
                        if (watch.Elapsed.TotalSeconds > 1)
                            Console.WriteLine($"[{recipe.Id}] 连接/全域复用模型 {choices.Length}放置 可行 {watch.Elapsed.TotalSeconds:F3}秒。");
                        return (card, status);
                    }
                }
                if (watch.Elapsed.TotalSeconds >= budget)
                    return (null, CpSolverStatus.Unknown);
            }
            model.ClearAssumptions();
        ...
        while (watch.Elapsed.TotalSeconds < budget)
        {
            ...
            if (linear)
                (chosen, status) = SolveLinear(model, selected, budget - watch.Elapsed.TotalSeconds);
            else
            {
            ...
            {
                if (watch.Elapsed.TotalSeconds > 1)
                    Console.WriteLine($"[{recipe.Id}] {(connect ? "连接" : "覆盖")}/{(compact ? "紧域" : "全域")} {choices.Length}放置 可行 {watch.Elapsed.TotalSeconds:F3}秒。");
                return (card, status);
            }
        }
        return (null, CpSolverStatus.Unknown);
