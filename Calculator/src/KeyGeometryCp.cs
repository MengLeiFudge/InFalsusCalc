...
internal sealed partial class KeyRecipeSolver
{
    ...
    private void AddConnection(CpModel model, BoolVar[] active, List<BoolVar>[] cover, HashSet<Hex> forced, RegionSet[] sets, IntVar splitPenalty)
    {
        ...
        {
            int node = enabled.Count;
            enabled.Add(model.NewConstant(1));
            foreach (Hex c in component)
                nodes[cellIndex[c]] = node;
        }
        int fixedNodes = enabled.Count;
        for (int c = 0; c < cells.Length; c++)
            if (nodes[c] < 0 && cover[c].Count > 0)
            {
                BoolVar used = model.NewBoolVar($"used{c}");
                model.AddMaxEquality(used, cover[c]);
                nodes[c] = enabled.Count;
                enabled.Add(used);
            }
        ...
        for (int axis = 0; axis < axes.Length; axis++)
        {
            ...
            for (int coordinate = slices.Keys.Min(); coordinate <= slices.Keys.Max(); coordinate++)
            {
                ...
                if (slices.TryGetValue(coordinate, out int[]? members))
                    model.AddMaxEquality(occupied, members.Select(n => enabled[n]));
                else
                    model.Add(occupied == 0);
                BoolVar start = model.NewBoolVar($"run{axis}_{coordinate}");
                model.Add(start >= occupied - previous);
                model.Add(start <= occupied);
        ...
        for (int i = 0; i < recipe.Areas.Length; i++)
        {
            ...
            foreach (var component in Craft.Components(recipe.Areas[i].Cells.Select(c => c.Position)))
            {
                ...
                if (n < 0)
                    continue;
                if (!areasAtNode.TryGetValue(n, out var list))
                    areasAtNode[n] = list = [];
                list.Add(i);
            }
        }
        ...
        foreach (var pair in areasAtNode)
        {
            ...
            model.AddMaxEquality(demand, pair.Value.Distinct().Select(i => active[i]));
            needed[pair.Key] = demand;
            model.Add(demand <= enabled[pair.Key]);
            BoolVar root = model.NewBoolVar($"root{pair.Key}");
            rootAtNode[pair.Key] = root;
            model.Add(root <= demand);
            roots.Add(root);
            if (pair.Key == fixedRoot)
                model.Add(root == 1);
            IntVar supply = model.NewIntVar(0, capacity, $"supply{pair.Key}");
            model.Add(supply <= capacity * root);
            flows[pair.Key].Add(supply);
        }
        ...
        for (int c = 0; c < cells.Length; c++)
        {
            ...
            if (nearRoots.Length == 0)
                continue;
            foreach (BoolVar particle in cover[c])
            {
                if (!touchingRoots.TryGetValue(particle, out var touched))
                    touchingRoots[particle] = touched = [];
        ...
        foreach (var pair in touchingRoots)
            if (pair.Value.Count > 1)
                model.Add(LinearExpr.Sum(pair.Value.Select(n => rootAtNode[n])) <= 1).OnlyEnforceIf(pair.Key);
        ...
        for (int c = 0; c < cells.Length; c++)
            if (nodes[c] >= 0)
                particlesAtNode[nodes[c]].UnionWith(cover[c]);
        HashSet<string> seen = [];
        int cuts = 0;
        ...
        if (needed.Count <= 12)
        {
            ...
            foreach (var terminal in needed.Where(p => p.Key != fixedRoot))
            {
                ...
                foreach (var root in rootAtNode)
                {
                    ...
                    model.Add(supply <= root.Value);
                    model.Add(supply <= terminal.Value);
                ...
                foreach ((int a, int b) in edges)
                {
                    ...
                    model.Add(flow <= enabled[a]);
                    model.Add(flow >= -enabled[a]);
                    model.Add(flow <= enabled[b]);
                    model.Add(flow >= -enabled[b]);
                    route[a].Add(-flow);
                    route[b].Add(flow);
                }
        ...
        foreach ((int a, int b) in edges)
        {
            ...
            model.Add(flow <= capacity * enabled[a]);
            model.Add(flow >= -capacity * enabled[a]);
            model.Add(flow <= capacity * enabled[b]);
            model.Add(flow >= -capacity * enabled[b]);
            flows[a].Add(-flow);
            flows[b].Add(flow);
        }
