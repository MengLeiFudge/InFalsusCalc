...
internal sealed class Battle
{
    ...
    {
        if (player.Length != 5)
            throw new ArgumentException("需要五张卡。", nameof(player));
        if (rating is < 1 or > 20)
            throw new ArgumentOutOfRangeException(nameof(rating), "等级应为1至20。");
        if (encounter.NearChance != 0 || encounter.MissChance != 0)
            throw new InvalidDataException("该回想存在未支持的随机敌方判定。");
        this.catalog = catalog;
        this.encounter = encounter;
        this.rating = rating;
        decks = [player, encounter.Cards];
        ...
        for (int side = 0; side < 2; side++)
            for (int slot = 0; slot < 5; slot++)
            {
                ...
                if (side == 0)
                    multiplier *= playerBonus;
                baseStats[side, 0] += card.Power * multiplier;
                baseStats[side, 1] += card.Fortitude * multiplier;
                for (int traitSlot = 0; traitSlot < card.Traits.Length; traitSlot++)
                {
                    int tid = card.Traits[traitSlot];
                    if (tid <= 1)
                        continue;
                    ...
                    for (int i = 0; i < trait.Effects.Length; i++)
                    {
                        ...
                        if (e.Kind is not (1 or 2 or 3 or 4 or 5 or 128 or 129 or 8096 or 8097))
                            throw new InvalidDataException($"未支持的特性效果{e.Kind}。");
    ...
    private (double[] Attack, double[] Defense, double[] PlainDefense, double[] Critical) Stats()
    {
        ...
        for (int side = 0; side < 2; side++)
        {
            ...
            foreach (Modifier modifier in modifiers[side].Values)
                ...
                {
                    case 1:
                        pUp += modifier.Value * .01;
                        break;
                    case 2:
                        fUp += modifier.Value * .01;
                        break;
                    case 3 or 129:
                        fDown += modifier.Value / 100;
                        break;
                    case 128:
                        pDown += modifier.Value / 100;
                        break;
                    case 5:
                        crit = Math.Max(0, crit) + modifier.Value / 100;
                        break;
                    case 4:
                        isolation[side] |= decks[1 - side][phase].Color != modifier.Value;
                        break;
                }
            if (fast)
            {
                attack[side] = baseStats[side, 0] * (pUp / pDown);
                defense[side] = baseStats[side, 1] * (fUp / fDown);
                plain[side] = baseStats[side, 1];
            }
            else
            {
    ...
    private void Damage(int target, double amount)
    {
        if (!double.IsFinite(amount) || amount < 0)
            throw new InvalidDataException("出现无效伤害。");
        hp[target] -= amount;
        damage[target] += amount;
        if (encounter.Mode == "connect" && hp[target] <= 0)
            broken[target] = true;
    ...
    private void Trigger(int side, Effect effect)
    {
        ...
        if (effect.Kind == 8096)
        {
            var values = Stats();
            int other = 1 - side;
            if (values.Defense[other] <= 0 || values.PlainDefense[other] <= 0)
                throw new InvalidDataException("额外攻击遇到未核对的零防御条件。");
        ...
        {
            if (encounter.Mode == "connect" && broken[side])
                amount = 0;
            else
            {
                double next = Math.Min(100, hp[side] + amount);
                amount = next - hp[side];
                hp[side] = next;
                healing[side] += amount;
            }
        ...
        if (keepTrace)
            events.Add(new TraitEvent
            {
                At = notesDone,
                Phase = phase + 1,
                Side = side,
                Trait = effect.Trait,
                Kind = effect.Kind == 8096 ? "damage" : "heal",
                Amount = amount,
                Hp = (double[])hp.Clone()
            });
    }
    ...
    private void ProcessEvents(int current, bool ending)
    {
        ...
        for (int index = 0; index < queue.Count; index++)
        {
            ...
            if (e.Kind is 1 or 2)
            {
                ...
                {
                    Effect effect = effects[e.Side][i];
                    BattleCard card = decks[e.Side][effect.Owner];
                ...
                Trigger(e.Side, effects[e.Side][e.Index]);
            else if (e.Kind == 4)
                modifiers[e.Side][e.Identity] = e.Modifier;
            else if (e.Kind == 5)
    ...
    public BattleResult Run(bool exact = false, bool trace = false)
    {
        fast = !exact;
        keepTrace = trace;
        const int count = Notes / 5, threshold = count * 4 / 5;
        for (int current = 0; current < 5; current++)
        {
            phase = current;
            double[] before = (double[])hp.Clone(), damageBefore = (double[])damage.Clone(), healBefore = (double[])healing.Clone();
            ...
            if (exact)
                ...
                {
                    Damage(1, outgoing * (note < threshold ? 1 : values.Critical[0]));
                    Damage(0, incoming);
                    notesDone++;
                }
            else
            {
                Damage(1, outgoing * (threshold + (count - threshold) * values.Critical[0]));
                Damage(0, incoming * count);
                notesDone += count;
            }
            ...
            if (trace)
                phases.Add(new PhaseResult
                {
                    ...
                    Before = before,
                    After = (double[])hp.Clone(),
                    Damage = [damage[0] - damageBefore[0], damage[1] - damageBefore[1]],
                    Healing = [healing[0] - healBefore[0], healing[1] - healBefore[1]]
                });
        }
        ...
        return new BattleResult
        {
            ...
            Margin = encounter.Mode == "reflection" ? progress - 100 : Math.Min(-hp[1], minimum),
            Progress = progress,
            Hp = (double[])hp.Clone(),
            MinimumHp = minimum,
            Broken = (bool[])broken.Clone(),
            EnemyProgress = 100 - hp[1],
            PlayerRemaining = Math.Max(0, hp[0]),
            EnemyRemaining = Math.Max(0, hp[1]),
            Damage = (double[])damage.Clone(),
            Healing = (double[])healing.Clone(),
            Phases = phases.ToArray(),
            Events = events.ToArray(),
            ExactAccumulation = exact
        };
    }
}
...
internal sealed class BattleResult
{
    ...
    public double EnemyProgress
    {
        get; init;
    }
    /// <summary>己方显示用剩余HP。</summary>
    public double PlayerRemaining
    {
        get; init;
    }
    /// <summary>对方显示用剩余HP。</summary>
    public double EnemyRemaining
    {
        get; init;
    }
