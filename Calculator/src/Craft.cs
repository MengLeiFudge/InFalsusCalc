...
internal static class Craft
{
    ...
    public static List<HashSet<Hex>> Components(IEnumerable<Hex> cells)
    {
        ...
        {
            Hex first = remaining.First();
            remaining.Remove(first);
            HashSet<Hex> group = [first];
            Stack<Hex> stack = new();
            stack.Push(first);
    ...
    public static CardTemplate Evaluate(Catalog catalog, Recipe recipe, IEnumerable<Placement> input)
    {
        ...
        Dictionary<Hex, int> occupied = [];
        HashSet<(Hex, int)> matching = [];
        ...
        {
            Shape shape = catalog.Shapes[raw.Id];
            Hex at = new(raw.Q, raw.R);
            Hex[] cells = shape.Cells.Select(c => c.Add(at)).OrderBy(c => c.Q).ThenBy(c => c.R).ToArray();
            if (cells.Any(c => !board.Contains(c)))
                throw new InvalidDataException($"配方{recipe.Id}有粒子超出合法外框。");
            foreach (Hex cell in cells)
            {
                occupied[cell] = occupied.GetValueOrDefault(cell) + 1;
                matching.Add((cell, shape.Color));
            }
            if (cells.Any(c => !safe.Contains(c)))
                outside++;
            tiers[shape.Tier - 1]++;
            profiles[shape.Profile]++;
            selected.Add(new Placement { Id = shape.Id, Q = raw.Q, R = raw.R, Color = shape.Color, Cells = cells });
        }
        int[] effects = new int[16];
        List<int> active = [];
        HashSet<int> colors = [recipe.BaseColor];
        int left = 0, right = 0;
        for (int i = 0; i < recipe.Areas.Length; i++)
        {
            BonusArea area = recipe.Areas[i];
            if (!area.Cells.All(c => matching.Contains((c.Position, c.Color))))
                continue;
            active.Add(i);
            foreach (RegionEffect effect in area.Effects)
            {
                if (effect.Kind == 1)
                    colors.Add(effect.Arguments[0].Value);
                else if (effect.Kind == 8)
                {
                    left += effect.Arguments[0].Value;
                    right += effect.Arguments[1].Value;
                }
                else if (effect.Kind is 3 or 4 or 7 or 9 or 10 or 11 or 12 or 15)
                    effects[effect.Kind] += effect.Arguments[0].Value;
                else
                    throw new InvalidDataException($"未支持的区域效果{effect.Kind}。");
            }
        }
        var limits = Skills[recipe.Character];
        int limit = Math.Min(limits.Count, recipe.MaxIota);
        ...
        CardTemplate card = new()
        {
            ...
            Fortitude = fortitude,
            BasePower = effects[3],
            BaseFortitude = effects[4],
            Slots = slots,
            Left = left,
            Right = right,
            InnerStructure = innerStructure,
            OuterStructure = outerStructure,
            Colors = colors.Order().ToArray(),
            TierCounts = tiers,
            AvailableTraits = available,
            RetainedPercent = Math.Max(0, (9d - strikes * strikes) / 9) * 100,
            Carriers = profiles.Select(n => Math.Min(3, n)).ToArray(),
            Placements = selected.ToArray(),
            Active = active.ToArray(),
            Strikes = strikes,
            Penalties = penalties,
            RawPenalties = rawPenalties,
            Falsehood = falsehood,
            Valid = power > 0 && fortitude > 0 && !falsehood && !overstacked,
            Group = group
        };
    ...
    private static int[] Masks(MaterialProfile profile, int[] wanted)
    {
        ...
        for (int mask = 1; mask < 1 << wanted.Length; mask++)
        {
            ...
            if (profile.Options.Any(p => p.Capacity >= skills.Length && skills.All(p.Traits.Contains)))
                masks.Add(mask);
        }
    ...
    public static AssignedMaterial[]? Assign(Catalog catalog, CardTemplate card, int[] traits)
    {
        ...
        if (wanted.Length == 0)
            return [];
        if (wanted.Length > card.Slots)
            return null;
        int full = (1 << wanted.Length) - 1;
        Dictionary<int, List<AssignedMaterial>> states = new()
        {
            [0] = []
        };
        for (int index = 0; index < card.Placements.Length; index++)
        {
            ...
            foreach (var prior in states.ToArray())
                ...
                {
                    int combined = prior.Key | mask;
                    if (states.ContainsKey(combined))
                        continue;
                    ...
                    if (combined == full)
                        return states[combined].ToArray();
                }
        }
        return null;
    }
