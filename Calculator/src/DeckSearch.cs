...
internal sealed record DeckChoice(CardTemplate Template, int Color, int[] Traits)
{
    ...
    public BattleCard ToBattleCard() => new()
    {
        Name = Template.Name,
        Color = Color,
        Power = Template.Power,
        Fortitude = Template.Fortitude,
        Left = Template.Left,
...
internal sealed class DeckResult
{
    ...
    public required BattleResult Battle
    {
        get; init;
    }
...
internal sealed class DeckSearch
{
    ...
    {
        if (maxStrikes is < 0 or > 2)
            throw new ArgumentOutOfRangeException(nameof(maxStrikes), "允许罚分应为0至2。");
        ...
        if (this.templates.Select(c => c.BaseId).Distinct().Count() < 5)
            throw new InvalidDataException("二阶段筛选后不足五种不同名卡。");
                ...
                foreach (TraitEffect effect in catalog.Traits[tid].Effects.Where(e => e.Kind == 4))
                    for (int phase = 0; phase < 5; phase++)
                    {
                        ...
                        if (effect.Condition switch
                        {
                            ...
                            isolation[phase].Add((int)effect.Value);
                    }
    }
    ...
    private static bool Better(BattleResult candidate, BattleResult? previous)
    {
        ...
        if (first != old)
            return first > old;
        if (candidate.Passed && candidate.RawScore != previous.RawScore)
            return candidate.RawScore > previous.RawScore;
    ...
    private bool Consider(DeckChoice[] candidate)
    {
        ...
        if (candidate.Select(c => c.Template.BaseId).Distinct().Count() != 5)
            return false;
        ...
        localBest = value;
        team = candidate;
        if (value.Passed && Better(value, winningBattle))
        {
            winningBattle = value;
            winner = (DeckChoice[])candidate.Clone();
        }
        return true;
    }
    ...
    private int[][] Combinations(CardTemplate card)
    {
        ...
        if (combinationCache.TryGetValue(key, out int[][]? cached))
            return cached;
        List<int[]> result = [[]];
        for (int a = 0; a < allowed.Length && card.Slots > 0; a++)
        {
            ...
            {
                int[] two = [allowed[a], allowed[b]];
                result.Add(two);
                for (int c = b + 1; c < allowed.Length && card.Slots > 2; c++)
                    result.Add([allowed[a], allowed[b], allowed[c]]);
            }
        }
    ...
    private (int[] Traits, double Gain)[] BestSkills(int slot, CardTemplate card, double offense, double defense)
    {
        ...
        foreach (int tid in allowed)
        {
            ...
            {
                double share = e.Condition switch
                {
                    4 => .8,
                    5 => 1 - width / 5d,
                    6 => .2,
                ...
                if (e.Kind is 1 or 129)
                    gain += e.Value / 100 * share * offense;
                else if (e.Kind is 2 or 128)
                    gain += e.Value / (100 + e.Value) * share * defense;
                else if (e.Kind == 3)
                    gain -= e.Value / 100 * share * defense;
                else if (e.Kind is 8096 or 8097)
                {
                    int count = e.Condition is 2 or 9 ? width : 1;
                    if (e.Kind == 8097)
                    {
                        if (e.Condition == 8 && start == 0 || e.Condition == 10 && slot == 0)
                            count = 0;
                        else if (e.Condition == 9 && start == 0)
                            count--;
                    }
    ...
    private DeckChoice[] Pool(int slot)
    {
        ...
        {
            if (localBest.MinimumHp <= 0 && localBest.EnemyProgress >= 100)
                offense = .25;
            else if (localBest.MinimumHp > 0 && localBest.EnemyProgress < 100)
                defense = .35;
        }
        ...
        PriorityQueue<DeckChoice, double> best = new();
        Dictionary<int, (DeckChoice Choice, double Score)> byName = [];
        foreach (CardTemplate card in templates)
        {
            token.ThrowIfCancellationRequested();
            string signature = $"{card.Slots},{card.Left},{card.Right}:{string.Join(',', card.Carriers)}";
            if (!skillsCache.TryGetValue(signature, out var skills))
                skillsCache[signature] = skills = BestSkills(slot, card, offense, defense);
            foreach (int color in card.Colors)
            {
                ...
                if (isolation[slot].Count == 1 && !isolation[slot].Contains(color))
                    score -= .3 * offense;
                int newDiversity = others.Count + (color != 0 && !others.Contains(color) ? 1 : 0);
                score += 2 * ((1 + .05 * newDiversity) / (1 + .05 * diversity) - 1);
                foreach (var skill in skills)
                {
                    DeckChoice choice = new(card, color, skill.Traits);
                    double value = score + skill.Gain;
                    best.Enqueue(choice, value);
                    if (best.Count > 64)
                        best.Dequeue();
                    if (!byName.TryGetValue(card.BaseId, out var old) || value > old.Score)
                        byName[card.BaseId] = (choice, value);
                }
            }
        }
    ...
    private DeckChoice[] Seed(int goal)
    {
        ...
        for (int slot = 0; slot < 5; slot++)
        {
            ...
            used.Add(selected.Template.BaseId);
            result.Add(selected);
        }
    ...
    private Dictionary<(int Slot, int BaseId, int Goal), DeckChoice[]> SeedOptions()
    {
        ...
        foreach (int slot in Enumerable.Range(0, 5))
            foreach (int baseId in templates.Select(c => c.BaseId).Distinct())
                ...
                {
                    (double offense, double defense) = goal switch
                    {
                        0 => (1, .15),
                        1 => (.15, 1),
                        2 => (1, 1),
        ...
        IEnumerable<int[]> Walk(int depth)
        {
            ...
            for (int i = 0; i < names.Length; i++)
            {
                if (used[i])
                    continue;
                used[i] = true;
                current[depth] = names[i];
                foreach (int[] value in Walk(depth + 1))
                    yield return value;
                used[i] = false;
            }
        }
    ...
    {
        HashSet<int> colors = [];
        DeckChoice[] result = new DeckChoice[5];
        for (int slot = 0; slot < 5; slot++)
        {
            ...
            if (result[slot].Color != 0)
                colors.Add(result[slot].Color);
        }
        return result;
    }
    ...
    private void AddBeam(DeckChoice[] candidate, PriorityQueue<DeckChoice[], double> beam, HashSet<string> seen)
    {
        ...
        beam.Enqueue((DeckChoice[])candidate.Clone(), priority);
        if (beam.Count > 256)
            beam.Dequeue();
    }
    ...
    private DeckChoice[][] SkillBeam(DeckChoice[] seed)
    {
        ...
        for (int slot = 0; slot < 5; slot++)
        {
            ...
            PriorityQueue<DeckChoice[], double> next = new();
            HashSet<string> seen = [];
            foreach (DeckChoice[] state in states)
                foreach (int[] skills in packages)
                {
                    DeckChoice[] candidate = (DeckChoice[])state.Clone();
                    candidate[slot] = candidate[slot] with
                    {
                        Traits = skills
                    };
                    ...
                    if ((evaluated & 63) == 0)
                        token.ThrowIfCancellationRequested();
                    evaluated++;
                    BattleResult value = new Battle(catalog, candidate.Select(c => c.ToBattleCard()).ToArray(), encounter).Run(false);
                    double priority = value.Passed ? 1e12 + value.RawScore : value.Margin;
                    next.Enqueue(candidate, priority);
                    if (next.Count > 24)
                        next.Dequeue();
                }
    ...
    private void Improve(DeckChoice[] seed)
    {
        ...
        for (int round = 0; round < 5; round++)
        {
            ...
            for (int slot = 0; slot < 5; slot++)
            {
                ...
                {
                    if (team.Where((_, i) => i != slot).Any(c => c.Template.BaseId == choice.Template.BaseId))
                        continue;
                    DeckChoice[] candidate = (DeckChoice[])team.Clone();
                    candidate[slot] = choice;
                    Consider(candidate);
                }
            }
                    ...
                    foreach (DeckChoice a in pools[left].Take(24))
                        foreach (DeckChoice b in pools[right].Take(24))
                        {
                            if (a.Template.BaseId == b.Template.BaseId || team.Where((_, i) => i != left && i != right)
                                .Any(c => c.Template.BaseId == a.Template.BaseId || c.Template.BaseId == b.Template.BaseId))
                                continue;
                            DeckChoice[] candidate = (DeckChoice[])team.Clone();
                            candidate[left] = a;
                            candidate[right] = b;
    ...
    public DeckResult? Run(IEnumerable<DeckChoice[]>? initialTeams = null)
    {
        ...
        PriorityQueue<DeckChoice[], double> beam = new();
        HashSet<string> seen = [];
        IEnumerable<DeckChoice[]> supplied = initialTeams ?? [];
        DeckChoice[]? known = KnownSeed();
        if (known is not null)
            supplied = supplied.Append(known);
        foreach (DeckChoice[] seed in Enumerable.Range(0, 4).Select(Seed).Concat(supplied))
    ...
    private DeckResult? Result(Stopwatch timer)
    {
        ...
        if (!battle.Passed)
            throw new InvalidDataException("保存前复算未满足通关条件。");
        DeckCardResult[] cards = winner.Select(c => new DeckCardResult
        {
            Template = c.Template.Id,
            Color = c.Color,
            Traits = c.Traits,
            Materials = Craft.Assign(catalog, c.Template, c.Traits) ?? throw new InvalidDataException("技能不能由实际粒子同时提供。")
        }).ToArray();
    ...
    internal static IEnumerable<int[]> Permutations(int[] values)
    {
        ...
        for (int i = 0; i < values.Length; i++)
            foreach (int[] tail in Permutations(values.Where((_, index) => index != i).ToArray()))
