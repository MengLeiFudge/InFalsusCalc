...
internal sealed partial class KeyRecipeSolver
{
    ...
    private (CardTemplate? Card, CpSolverStatus Status) PatternGraph(RegionSet set, double seconds)
    {
        ...
        if (regions.Any(i => Craft.Components(recipe.Areas[i].Cells.Select(c => c.Position)).Count != 1))
            return (null, CpSolverStatus.Unknown);
        List<Option[]> pools = [];
        ...
        if (bridges > 0)
        {
            ...
            for (int r = 0; r < pools.Count; r++)
                foreach (Hex c in pools[r].SelectMany(p => p.Cells).Distinct().SelectMany(c => Hex.Directions.Select(cells[c].Add).Append(cells[c])))
                    if (cellIndex.TryGetValue(c, out int id))
                        near[id] = near.GetValueOrDefault(id) | (1u << r);
        ...
        for (int i = 0; i < pools.Count; i++)
        {
            ...
            for (int j = 0; j < pools[i].Length; j++)
            {
                ...
                Option p = pools[i][j];
                count.Add(chosen[i][j] * p.Layout.Length);
                outside.Add(chosen[i][j] * p.Outside);
                foreach (int c in p.Cells)
                    cover[c].Add(chosen[i][j]);
                foreach (int b in p.Matches)
                    matches[b].Add(chosen[i][j]);
            }
            if (i < regions.Length)
                model.Add(used[i] == 1);
            else
            {
                model.Add(choice[i] >= 1).OnlyEnforceIf(used[i]);
                model.Add(choice[i] == 0).OnlyEnforceIf(used[i].Not());
            }
        }
        if (bridges == 2)
            model.Add(choice[^2] <= choice[^1]);
        model.Add(LinearExpr.Sum(count) <= countLimit);
        model.Add(LinearExpr.Sum(outside) <= outsideLimit);
        ...
        for (int r = 0; r < recipe.Areas.Length; r++)
        {
            ...
            foreach (BoardCell cell in recipe.Areas[r].Cells)
            {
                if ((set.Mask & (1u << r)) == 0)
                {
                    ...
                    model.Add(LinearExpr.Sum(matches[bit]) == 0).OnlyEnforceIf(absent);
                    missing.Add(absent);
                }
                bit++;
            }
            if (missing.Count > 0)
                model.AddBoolOr(missing);
        }
        ...
        {
            BoolVar root = model.NewBoolVar($"root{i}");
            roots.Add(root);
            if (i == 0)
                model.Add(root == 1);
            IntVar source = model.NewIntVar(0, capacity, $"source{i}");
            model.Add(source <= capacity * root);
            flows[i].Add(source);
        }
            ...
            for (int b = a + 1; b < pools.Count; b++)
            {
                ...
                for (int x = 0; x < pools[a].Length; x++)
                    for (int y = 0; y < pools[b].Length; y++)
                        if (pools[b][y].Cells.Any(adjacent[a][x].Contains))
                            touching.Add((x, y));
                if (touching.Count == 0)
                    continue;
                BoolVar edge = model.NewBoolVar($"edge{a}_{b}");
                model.Add(edge <= used[a]);
                model.Add(edge <= used[b]);
                incidence[a].Add(edge);
                incidence[b].Add(edge);
                neighbors[a].Add(b);
                neighbors[b].Add(a);
                long[,] tuples = new long[touching.Count, 2];
                for (int i = 0; i < touching.Count; i++)
                {
                    tuples[i, 0] = touching[i].Item1;
                    tuples[i, 1] = touching[i].Item2;
                }
        ...
        CpSolver solver = new()
        {
            StringParameters = FormattableString.Invariant($"max_time_in_seconds:{seconds} num_search_workers:{workers} cp_model_probing_level:0 linearization_level:0")
        };
        ...
        if (!card.Valid || card.Strikes != 0 || actual != set.Mask)
            throw new InvalidDataException($"区域图复核不一致：{recipe.Id}/{set.Mask}/{actual}。");
        return (card, status);
    }
}
