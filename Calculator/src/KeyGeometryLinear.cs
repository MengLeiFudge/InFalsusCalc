...
internal sealed partial class KeyRecipeSolver
{
    ...
    private (int[] Chosen, CpSolverStatus Status) SolveLinear(CpModel model, BoolVar[] selected, double budget)
    {
        ...
        if (!double.IsPositiveInfinity(budget))
            solver.SetTimeLimit((long)Math.Max(1, budget * 1000));
        solver.SetSolverSpecificParametersAsString("limits/solutions = 1\ndisplay/verblevel = 0");
        List<MipVariable> vars = [];
        List<(double Lo, double Hi)> bounds = [];
        foreach (IntegerVariableProto v in model.Model.Variables)
        {
            if (v.Domain.Count != 2)
                throw new InvalidDataException("线性后端尚不支持带孔变量域。");
        ...
        void Row(Dictionary<int, double> terms, double offset, double lower, double upper, IEnumerable<int> guards)
        {
            ...
            foreach (bool low in new[] { true, false })
            {
                ...
                if (double.IsInfinity(value))
                    continue;
                Dictionary<int, double> row = new(terms);
                ...
                foreach (int literal in literals)
                {
                    ...
                    {
                        constant += sign * slack;
                        Add(row, literal, -sign * slack);
                    }
                    else
                        Add(row, -literal - 1, sign * slack);
                }
                ...
                if (term.Value != 0)
                    constraint.SetCoefficient(vars[term.Key], term.Value);
            }
        }
        foreach (ConstraintProto c in model.Model.Constraints)
        {
            ...
            switch (c.ConstraintCase)
            {
                case ConstraintProto.ConstraintOneofCase.Linear:
                    if (c.Linear.Domain.Count != 2)
                        throw new InvalidDataException("线性后端尚不支持带孔线性约束。");
                    Dictionary<int, double> linear = [];
                    for (int i = 0; i < c.Linear.Vars.Count; i++)
                        Add(linear, c.Linear.Vars[i], c.Linear.Coeffs[i]);
                ...
                case ConstraintProto.ConstraintOneofCase.ExactlyOne:
                    ...
                    Dictionary<int, double> clause = [];
                    double offset = 0;
                    foreach (int literal in literals)
                        if (literal >= 0)
                            Add(clause, literal, 1);
                        else
                        {
                            Add(clause, -literal - 1, -1);
                            offset++;
                        }
                ...
                case ConstraintProto.ConstraintOneofCase.LinMax:
                    ...
                    if (targetRange.Lo < 0 || targetRange.Hi > 1)
                        throw new InvalidDataException("仅支持布尔最大值。");
                    foreach (LinearExpressionProto e in max.Exprs)
                    {
                        var terms = Terms(e);
                        var range = Range(terms, e.Offset);
                        if (range.Lo < 0 || range.Hi > 1)
                            throw new InvalidDataException("仅支持布尔最大值参数。");
                        Dictionary<int, double> comparison = new(target);
                        foreach (var t in terms)
                        {
                            Add(comparison, t.Key, -t.Value);
                            Add(sum, t.Key, -t.Value);
                        }
                ...
                case ConstraintProto.ConstraintOneofCase.Table:
                    if (c.Table.Negated || c.Table.Exprs.Count == 0)
                        throw new InvalidDataException("仅支持显式允许表达式表。");
                    int width = c.Table.Exprs.Count, count = c.Table.Values.Count / width;
                    if (c.Table.Values.Count % width != 0)
                        throw new InvalidDataException("区域表维度不一致。");
                    int[] cases = Enumerable.Range(0, count).Select(i =>
                    {
                        int index = vars.Count;
                        vars.Add(solver.MakeBoolVar($"table{index}"));
                        bounds.Add((0, 1));
                        return index;
                    }).ToArray();
                    ...
                    {
                        var e = c.Table.Exprs[column];
                        var terms = Terms(e);
                        for (int i = 0; i < count; i++)
                            Add(terms, cases[i], -c.Table.Values[i * width + column]);
        ...
        {
            for (int i = 0; i < objective.Vars.Count; i++)
                solver.Objective().SetCoefficient(vars[objective.Vars[i]], objective.Coeffs[i]);
        ...
        Mip.ResultStatus status = solver.Solve();
        cancellation.ThrowIfCancellationRequested();
        Console.WriteLine($"[{recipe.Id}] 连续流后端：{status}，{watch.Elapsed.TotalSeconds:F3}秒。");
        if (status == Mip.ResultStatus.INFEASIBLE)
            return ([], CpSolverStatus.Infeasible);
        if (status is not (Mip.ResultStatus.FEASIBLE or Mip.ResultStatus.OPTIMAL))
