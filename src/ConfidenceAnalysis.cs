...
internal static class ConfidenceAnalysis
{
    ...
    public static int[][] RetainedKeys(Recipe recipe)
    {
        ...
        foreach (ConfidenceKey key in ranked.Take(minimum)) selected.Add(key.Key);
    ...
    public static void Migrate(Catalog catalog, Recipe recipe)
    {
        ...
        if (saved is null || saved.SelectionScope == Scope) return;
        GroupState[] zero = saved.Groups.Values.Where(g => g.Key.Length == 3 && g.Key[0] == 0).ToArray();
        if (zero.Length > 1)
        {
            GroupState merged = new() { Key = [0, 0, 0] };
    ...
    public static string Run(Catalog catalog)
    {
        ...
        var recipes = catalog.Data.Recipes.OrderBy(r => r.Id).Select(recipe =>
        {
            ...
            var regions = recipe.Areas.Select((area, index) =>
            {
                ...
                return new { area = index, slots, left, right };
        ...
        var globalRangeReports = globalRecipes.Select(recipe =>
        {
            ...
            var encounters = benchmark.Select(e =>
            {
                ...
                return new { encounter = e.Encounter, kept, omitted, omitted_exceeds = omitted is not null && Better(omitted, kept), strict = omitted is null || Better(kept, omitted) };
            }).ToArray();
            return new
            {
                recipe.recipe,
        ...
        lines.Add(""); lines.Add("70%/80%相对理想值分档：");
        ...
        lines.Add(""); lines.Add("全部配方理论结构 → 分析报告临时双路线：");
        foreach (var recipe in recipes) lines.Add($"{recipe.recipe:00} {recipe.name}：{string.Join(' ', recipe.keys)} → {string.Join(' ', recipe.retained_keys)}");
        Directory.CreateDirectory(Path.GetDirectoryName(textPath)!); File.WriteAllLines(textPath, lines);
    ...
    {
        Dictionary<(int Slots, int Left, int Right), Panel> power = new() { [(0, 0, 0)] = new(0, 0) };
        ...
        {
            if ((excluded & (1u << region)) != 0) continue;
        ...
        foreach (var raw in power.Keys.Union(fortitude.Keys).Union(total.Keys))
        {
            ...
            if (!result.TryGetValue(key, out var old)) old = (new(), new(), new());
    ...
    private static int[][] Combinations(int[] traits, int slots)
    {
        ...
        {
            if (selected.Count == slots) { result.Add(selected.ToArray()); return; }
            for (int i = start; i <= traits.Length - (slots - selected.Count); i++)
            {
                selected.Add(traits[i]); Pick(i + 1, selected); selected.RemoveAt(selected.Count - 1);
            }
        }
        Pick(0, []); return result.ToArray();
    }
    ...
    private static object? PanelSlotFrontier(Recipe recipe)
    {
        ...
        {
            if (!pair.Value.Best.TryGetValue("total", out string? id) || !result.Cards.TryGetValue(id, out CardTemplate? card)) return null;
            return new { key = pair.Key, card.Slots, card.Left, card.Right, width = Math.Min(4, card.Left + card.Right), card.Power, card.Fortitude, total = card.Total };
        ...
        return new { recipe = recipe.Id, name = recipe.Name, routes = frontier };
    }
}
