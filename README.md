...
网页资源采用独立文件，需通过 GitHub Pages 或静态网站服务访问，不支持双击 HTML 离线打开。首批数据只有卡牌摘要和文件索引；点击卡牌后才加载拼法，进入回想页才加载当前回想与技能参数；浏览器按当前谱面等级和总判定数结算，展开阶段或特性明细时展示本次计算结果。读取过的数据会复用缓存，相同内容的数据块共享文件。
