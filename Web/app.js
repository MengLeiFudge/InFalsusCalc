...
const goalNames={power:"攻击最高（同攻取防御最高）",fortitude:"防御最高（同防取攻击最高）",total:"攻防之和最高"};
...
const state={recipeIds:new Set(),tiers:new Set(),colors:new Set(),includeChromatic:false,libraryResults:[],libraryPage:0,result:null,layout:null,piece:0,layoutScale:1,layoutX:0,layoutY:0,layoutDrag:null};
const publishedCards=Object.values(DATA?.templates||{});
/** 兼容旧报告，按同一卡、同结构的最终攻防支配规则还原卡牌库。 */
function libraryDominates(candidate,other){
  const sameStructure=candidate.recipe===other.recipe&&candidate.slots===other.slots&&candidate.left===other.left&&candidate.right===other.right;
  const noWorse=candidate.power>=other.power&&candidate.fortitude>=other.fortitude;
  const strict=candidate.power>other.power||candidate.fortitude>other.fortitude;
  return candidate.id!==other.id&&sameStructure&&noWorse&&strict;
}
const libraryCards=DATA?.library_templates?Object.values(DATA.library_templates):publishedCards.filter(card=>!publishedCards.some(other=>libraryDominates(other,card)));
...
function traitTag(id,iconOnly=false){
  const item=trait(id),effect=item?.effects?.[0],icon=item&&DATA.catalog.trait_icons[item.icon],border=DATA.catalog.trait_border;
  const frame=effect&&DATA.catalog.trait_frames[String(effect.condition)],tierIcon=item&&DATA.catalog.trait_tiers[String(item.tier)];
  const details=item?.description?plain(item.description):"没有可用的效果说明。";
  const picture=icon?`<span class="trait-icon"><img class="trait-border" src="${border}" alt="">${frame?`<img class="trait-frame" src="${frame}" alt="">`:""}<img class="trait-effect" src="${icon}" alt="">${tierIcon?`<img class="trait-tier" src="${tierIcon}" alt="">`:""}</span>`:"";
...
function shapeIcon(id){
  const shape=DATA.catalog.shapes.find(item=>item.Id.Value===id),xy=(q,r)=>[1.5*q,Math.sqrt(3)*(r+q/2)];
  if(!shape)return "";
  const points=shape.Segments.map(cell=>xy(cell.Q,cell.R));
  const minX=Math.min(...points.map(point=>point[0]))-1.1,maxX=Math.max(...points.map(point=>point[0]))+1.1;
  const minY=Math.min(...points.map(point=>point[1]))-1.1,maxY=Math.max(...points.map(point=>point[1]))+1.1;
...
function encounterDrops(encounter){
  const loot=encounter.drops,groups=new Map();
  for(const drop of loot.items){const key=`${drop.min_potency}:${drop.max_potency}:${drop.trait_rolls}`;if(!groups.has(key))groups.set(key,{...drop,drops:[]});groups.get(key).drops.push(drop);}
  const pool=loot.traits.map(item=>`<span class="loot-trait">${traitTag(item.id)}<small>权重 ${item.weight} · ${num(item.weight/loot.trait_total_weight*100,2)}%</small></span>`).join("");
  const noTrait=loot.no_trait_weight?`<span class="loot-empty">无技能结果 · 权重 ${loot.no_trait_weight} · ${num(loot.no_trait_weight/loot.trait_total_weight*100,2)}%</span>`:"";
  const content=[...groups.values()].map(group=>`<div class="drop-group"><div class="drop-group-title"><strong>效能 ${group.min_potency}-${group.max_potency}</strong><span>${group.trait_rolls?`抽取技能 ${group.trait_rolls} 次`:"不抽取技能"}</span></div><div class="drop-shapes">${group.drops.map(drop=>`<span class="drop-shape" title="粒子${drop.shape} · ${colorNames[drop.color]} · ${drop.tier}阶 · ${drop.size}格"><span class="shape-picture">${shapeIcon(drop.shape)}</span><span>${colorNames[drop.color]} ${drop.tier}阶 · ${drop.size}格<small>权重 ${drop.weight} · ${num(drop.weight/loot.total_weight*100,2)}%</small></span></span>`).join("")}</div></div>`).join("");
  $("encounter-drops").innerHTML=`<div class="encounter-chapter"><span class="badge">${esc(encounter.chapter)}</span><span>掉落表 ${loot.table} · ${loot.items.length} 项粒子配置 · 总权重 ${loot.total_weight}</span></div><section class="loot-pool"><h4>对应技能池</h4><p>每次技能抽取独立按总权重 ${loot.trait_total_weight} 结算。</p><div class="loot-traits">${pool}${noTrait}</div></section>${content}`;
}
...
function fuzzyMatch(name,query){
  ...
  let position=0;
  for(const character of keyword){position=text.indexOf(character,position);if(position<0)return false;position++;}
  return true;
}
...
function updateRecipeChoices(){
  ...
  const eligible=new Set(libraryCards.filter(card=>matchesPrimary(card,recipeMap.get(card.recipe))).map(card=>card.recipe));
  for(const id of [...state.recipeIds])if(!eligible.has(id))state.recipeIds.delete(id);
  const query=$("recipe-search").value,choices=DATA.catalog.recipes.filter(recipe=>eligible.has(recipe.id)&&fuzzyMatch(recipe.name,query));
  $("selected-recipes").innerHTML=DATA.catalog.recipes.filter(recipe=>state.recipeIds.has(recipe.id)).map(recipe=>`<button type="button" class="multi-select-chip" data-remove-recipe="${recipe.id}" title="移除${esc(recipe.name)}"><span>${esc(recipe.name)}</span><b aria-hidden="true">&times;</b></button>`).join("");
  $("recipe-options").innerHTML=choices.map(recipe=>{const selected=state.recipeIds.has(recipe.id);return `<button type="button" class="multi-select-option${selected?" selected":""}" role="option" aria-selected="${selected}" data-recipe="${recipe.id}"><span>${esc(recipe.name)}<small>等级${recipe.tier}</small></span><b aria-hidden="true">&#10003;</b></button>`;}).join("")||'<p class="multi-select-empty">没有匹配的卡牌</p>';
}

/** 切换指定卡牌，不改变名称搜索内容。 */
function toggleRecipe(id){
  if(state.recipeIds.has(id))state.recipeIds.delete(id);else state.recipeIds.add(id);
  updateRecipeChoices();filterLibrary();$("recipe-search").focus();
}

/** 打开或关闭指定卡牌候选列表。 */
function setRecipeOptions(open){
  $("recipe-options").hidden=!open;$("recipe-search").setAttribute("aria-expanded",String(open));
}
...
function enumChoices(key){return key==="tiers"?[...new Set(DATA.catalog.recipes.map(recipe=>recipe.tier))].sort((a,b)=>a-b):Object.keys(colorNames).map(Number).filter(color=>color>0);}
function updateEnumFilter(key){
  const selected=state[key];
  $(enumFilters[key].options).innerHTML=enumChoices(key).map(value=>{
    const active=selected.has(value),content=key==="tiers"?`<img src="${DATA.catalog.card_assets.levels[String(value)]}" alt="">`:`<img src="${DATA.catalog.card_assets.icons[`${value}-0`]}" alt="">`;
...
function toggleEnumValue(key,value){
  const selected=state[key];if(selected.has(value))selected.delete(value);else selected.add(value);
  updateEnumFilter(key);updateRecipeChoices();filterLibrary();
}

/** 后级排序只有在前一级存在时才可用。 */
function updateSortControls(){
  for(let index=1;index<=3;index++)$("sort-direction-"+index).disabled=!$("sort-field-"+index).value;
  const thirdEnabled=Boolean($("sort-field-2").value);$("sort-field-3").disabled=!thirdEnabled;if(!thirdEnabled)$("sort-field-3").value="";
...
function sortLibraryCards(cards,recipeMap){
  const criteria=[];for(let index=1;index<=3;index++){const name=$("sort-field-"+index).value;if(!name)break;criteria.push([name,$("sort-direction-"+index).value]);}
  const field=(card,name)=>name==="order"?recipeOrder.get(card.recipe):name==="range"?card.left+card.right:name==="tier"?recipeMap.get(card.recipe).tier:card[name];
  return cards.sort((a,b)=>{
    for(const [name,direction] of criteria){const av=field(a,name),bv=field(b,name),difference=typeof av==="string"?av.localeCompare(bv,"zh-CN"):av-bv;if(difference)return direction==="desc"?-difference:difference;}
...
function renderLibraryPage(){
  const size=Number($("library-page-size").value),total=state.libraryResults.length,pages=Math.max(1,Math.ceil(total/size));
  state.libraryPage=Math.min(state.libraryPage,pages-1);
  const start=state.libraryPage*size,cards=state.libraryResults.slice(start,start+size);
  $("results").innerHTML=cards.length?cards.map(libraryCard).join(""):'<p class="empty-results">没有符合全部条件的卡牌。</p>';
  $("library-pagination").hidden=!total;$("library-page-prev").disabled=state.libraryPage===0;$("library-page-next").disabled=state.libraryPage>=pages-1;
  $("library-page-status").textContent=total?`${state.libraryPage+1} / ${pages} · ${start+1}-${start+cards.length}张，共${total}张`:"";
}

/** 所有启用条件以AND关系从统一最终模板库中筛选。 */
function filterLibrary(resetPage=true){
  const numeric=[["slots",card=>card.slots],["left",card=>card.left],["right",card=>card.right],["strikes",card=>card.strikes]];
  const recipeMap=new Map(DATA.catalog.recipes.map(recipe=>[recipe.id,recipe]));
  const cards=sortLibraryCards(libraryCards.filter(card=>{
    ...
    return !numeric.some(([id,value])=>!compareValue(value(card),$(id+"-op").value,Number($(id+"-value").value)));
  }),recipeMap);
  $("recipe-count").textContent=`${cards.length}张`;
  state.libraryResults=cards;if(resetPage)state.libraryPage=0;renderLibraryPage();
}

/** 恢复全部卡牌范围并清除所有并列条件。 */
function clearLibraryFilters(){
  $("recipe-search").value="";
  state.recipeIds.clear();state.tiers.clear();state.colors.clear();state.includeChromatic=false;
  $("include-chromatic").classList.remove("selected");$("include-chromatic").setAttribute("aria-pressed","false");
  for(const id of ["slots","left","right","strikes"]){$(id+"-op").value="";$(id+"-value").value="0";}
  $("sort-field-1").value="order";$("sort-direction-1").value="asc";$("sort-field-2").value="";$("sort-direction-2").value="desc";$("sort-field-3").value="";$("sort-direction-3").value="desc";$("library-page-size").value="20";updateSortControls();
  for(const key of Object.keys(enumFilters))updateEnumFilter(key);
  updateRecipeChoices();setRecipeOptions(false);filterLibrary();
}

/** 用同一套游戏资源合成玩家卡面；技能数组为空时保留空槽。 */
function cardVisual(card,color,traits=[],interactive=false){
  const recipe=DATA.catalog.recipes.find(item=>item.id===card.recipe),sr=recipe?.is_sr?1:0,slots=Math.max(1,Math.min(3,card.slots));
  const assets=DATA.catalog.card_assets,assetKey=`${color}-${sr}`,frame=assets.frames[`${assetKey}-${slots}`];
  const behavior=interactive?` role="button" tabindex="0" data-library-layout="${card.id}" aria-label="查看${esc(card.name)}详情与拼法"`:"";
  const slotsHtml=Array.from({length:card.slots},(_,index)=>traits[index]?traitTag(traits[index],true):`<span class="trait-empty"><img src="${DATA.catalog.trait_border}" alt="空技能槽"></span>`).join("");
  const pip=(side,index,x,value)=>`<img class="range-pip ${side}" style="left:${x}%" src="${assets.common[index<value?"medium-range-active":"medium-range-inactive"]}" alt="">`;
...
function selectEncounter(){
  const eid=Number($("encounter-select").value),maxStrikes=Number($("encounter-strikes").value),rating=Number($("encounter-rating").value)||DATA.defaults.search_rating;
  const encounter=DATA.catalog.encounters.find(e=>e.id===eid);
  const result=DATA.encounters[eid]?.[maxStrikes];
  if(!result)throw new Error("这个回想缺少所选罚分档位的预计算成果，成果文件不完整。");
  state.result=result;
  ...
  $("encounter-title").textContent=`${encounter.chapter} · ${encounter.name} · ${encounter.mode==="connect"?"连接":"映像"}`;
  encounterDrops(encounter);
  $("battle-condition").textContent=`允许罚分${maxStrikes} · 谱面等级${rating} · 配队搜索等级10 · 联觉开启 · 25个代表性全EXACT键 · 初始HP100`;
  ...
  $("events").innerHTML=b.events.length?b.events.map(e=>`<div class="event-row"><span>代表键进度 ${e.at}/25 · ${e.side===0?"己方":"对方"}</span>${traitTag(e.trait)}<span>· ${e.kind==="heal"?"有效回复":"造成伤害"} ${num(e.amount,4)}%</span></div>`).join(""):'<p class="help">没有瞬时攻击或回复事件。</p>';
}

/** 展示实际布局，素材来源编号沿用同一份已保存的粒子顺序。 */
function showLayout(identity,assignment=null,showGoals=true){
  const card=DATA.templates[identity];
  state.layout=card;state.piece=0;state.assignment=assignment;
  state.layoutScale=1;state.layoutX=0;state.layoutY=0;state.layoutDrag=null;
  const goalText=showGoals?(card.goals||[]).map(g=>goalNames[g]).join(" · "):"";$("layout-goals").textContent=goalText;$("layout-goals").hidden=!goalText;
  $("layout-title").textContent=card.name;
  $("layout-stats").textContent=`原始攻击 ${num(card.base_power)} · 原始防御 ${num(card.base_fortitude)} ｜ 卡牌攻击 ${num(card.power)} · 卡牌防御 ${num(card.fortitude)}`;
  const penaltyNames=["数量超限","越界","重叠","断连"],penaltyText=penaltyNames.map((name,index)=>[name,card.penalties[index]]).filter(([,value])=>value>0).map(([name,value])=>`${name}${value}`).join(" · ")||"无";
...
function drawLayout(){
  ...
  $("layout-board").innerHTML=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="${minX} ${minY} ${maxX-minX} ${maxY-minY}"><rect x="${minX}" y="${minY}" width="${maxX-minX}" height="${maxY-minY}" fill="#0c151d"/>${cells.join("")}</svg>`;
  applyLayoutView();
...
function zoomLayout(event){
  ...
  const board=$("layout-board"),bounds=board.getBoundingClientRect(),oldScale=state.layoutScale;
  const nextScale=Math.max(.5,Math.min(4,oldScale*Math.exp(-event.deltaY*.001))),ratio=nextScale/oldScale;
  const x=event.clientX-bounds.left-bounds.width/2,y=event.clientY-bounds.top-bounds.height/2;
  state.layoutX=x-(x-state.layoutX)*ratio;state.layoutY=y-(y-state.layoutY)*ratio;state.layoutScale=nextScale;
  applyLayoutView();
}
...
function moveLayoutPan(event){
  ...
  state.layoutX+=event.clientX-state.layoutDrag.x;state.layoutY+=event.clientY-state.layoutDrag.y;
  state.layoutDrag.x=event.clientX;state.layoutDrag.y=event.clientY;applyLayoutView();
}
function endLayoutPan(event){
  if(!state.layoutDrag||state.layoutDrag.pointer!==event.pointerId)return;
  state.layoutDrag=null;event.currentTarget.classList.remove("panning");
  if(event.currentTarget.hasPointerCapture(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId);
}

/** 用户主动保存当前拼法，SVG包含全部格子与编号。 */
function saveSvg(){
  const source=$("layout-board").querySelector("svg");if(!source)return;
  const svg=source.cloneNode(true);svg.style.removeProperty("transform");
...
function boot(){
  ...
  $("summary").textContent=`${DATA.catalog.recipes.length}张基础卡 · ${libraryCards.length}张最终卡 · ${DATA.catalog.encounters.length}个回想`;
  $("created").textContent=`生成于 ${new Date(DATA.created*1000).toLocaleString("zh-CN")}`;
  $("encounter-select").innerHTML=DATA.catalog.encounters.map(e=>`<option value="${e.id}">${esc(e.chapter)} · ${esc(e.name)} · ${e.mode==="connect"?"连接":"映像"}</option>`).join("");
  ...
  $("recipe-search").addEventListener("input",()=>{updateRecipeChoices();setRecipeOptions(true);});
  $("recipe-search").addEventListener("focus",()=>setRecipeOptions(true));
  $("recipe-search").addEventListener("keydown",event=>{
    if(event.key==="Escape"){setRecipeOptions(false);event.currentTarget.blur();}
    if(event.key==="Backspace"&&!event.currentTarget.value&&state.recipeIds.size){const ids=[...state.recipeIds];toggleRecipe(ids[ids.length-1]);}
  });
  $("recipe-multiselect").addEventListener("click",event=>{
    event.stopPropagation();
    const remove=event.target.closest("button[data-remove-recipe]"),option=event.target.closest("button[data-recipe]");
    if(remove){toggleRecipe(Number(remove.dataset.removeRecipe));return;}
    if(option){$("recipe-search").value="";toggleRecipe(Number(option.dataset.recipe));return;}
    $("recipe-search").focus();setRecipeOptions(true);
  });
  for(const [key,config] of Object.entries(enumFilters)){
    $(config.options).addEventListener("click",event=>{const button=event.target.closest("button[data-filter]");if(button)toggleEnumValue(key,Number(button.dataset.value));});
  }
  $("include-chromatic").addEventListener("click",event=>{
    state.includeChromatic=!state.includeChromatic;event.currentTarget.classList.toggle("selected",state.includeChromatic);event.currentTarget.setAttribute("aria-pressed",String(state.includeChromatic));
    updateRecipeChoices();filterLibrary();
  });
  for(const id of ["slots-op","slots-value","left-op","left-value","right-op","right-value","strikes-op","strikes-value"])
    $(id).addEventListener("change",filterLibrary);
  for(const id of ["sort-field-1","sort-direction-1","sort-field-2","sort-direction-2","sort-field-3","sort-direction-3"])$(id).addEventListener("change",()=>{updateSortControls();filterLibrary();});
  $("library-page-size").addEventListener("change",()=>{state.libraryPage=0;renderLibraryPage();});
  $("library-page-prev").addEventListener("click",()=>{if(state.libraryPage>0){state.libraryPage--;renderLibraryPage();$("recipe-count").scrollIntoView({block:"start"});}});
  $("library-page-next").addEventListener("click",()=>{const pages=Math.ceil(state.libraryResults.length/Number($("library-page-size").value));if(state.libraryPage+1<pages){state.libraryPage++;renderLibraryPage();$("recipe-count").scrollIntoView({block:"start"});}});
  ...
  $("encounter-strikes").addEventListener("input",()=>{try{selectEncounter();}catch(e){toast(e.message);}});
  ...
  layoutBoard.addEventListener("pointerdown",startLayoutPan);layoutBoard.addEventListener("pointermove",moveLayoutPan);
  layoutBoard.addEventListener("pointerup",endLayoutPan);layoutBoard.addEventListener("pointercancel",endLayoutPan);
  $("layout-flip").addEventListener("change",drawLayout);$("save-svg").addEventListener("click",saveSvg);
  $("results").addEventListener("keydown",event=>{
    const card=event.target.closest("[data-library-layout]");if(card&&(event.key==="Enter"||event.key===" ")){event.preventDefault();showLayout(card.dataset.libraryLayout,null,false);}
  });
  document.addEventListener("click",event=>{
    if(!event.target.closest("#recipe-multiselect"))setRecipeOptions(false);
    const libraryCard=event.target.closest("[data-library-layout]");if(libraryCard){showLayout(libraryCard.dataset.libraryLayout,null,false);return;}
  ...
  try{updateRecipeChoices();for(const key of Object.keys(enumFilters))updateEnumFilter(key);updateSortControls();filterLibrary();selectEncounter();}catch(error){toast(error.message);}
}
